...
namespace TwinsanityEditor
{
    public class RMViewer : ThreeDViewer
    {
        ...
        public RMViewer(FileController file, Form pform)
        {
            ...
            this.file = file;
            Tag = pform;
            if (file.Data.Type == TwinsFile.FileType.RM2 || file.Data.Type == TwinsFile.FileType.RMX || file.Data.Type == TwinsFile.FileType.DemoRM2 || file.Data.Type == TwinsFile.FileType.MonkeyBallRM)
            {
        ...
        protected override void RenderObjects()
        {
            ...
            //object visuals
            if ((file.Data.Type == TwinsFile.FileType.RM2 || file.Data.Type == TwinsFile.FileType.DemoRM2 || file.Data.Type == TwinsFile.FileType.RMX) && obj_models)
            {
                //GL.Enable(EnableCap.Lighting);
        ...
        private void LoadSceneryModel(SceneryData.SceneryModelStruct leaf, Matrix4 chunkMatrix)
        {
            ...
            else if (file.DataAux.Type == TwinsFile.FileType.SMX)
            {
                for (int m = 0; m < leaf.Models.Count; m++)
                {
                    ModelXController mesh;
                    uint modelID;
                    if (!leaf.Models[m].isSpecial)
                    {
                        modelID = leaf.Models[m].ModelID;
                        ...
                        foreach (InstanceDemo ins in file.Data.GetItem<TwinsSection>(i).GetItem<TwinsSection>(6).Records)
                        {
                            ...
                            if (file.Data.Type == TwinsFile.FileType.DemoRM2)
                            {
                                ...
                                if (TargetGI != 65535)
                                {
                                    ...
                                    if (HasSkin && file.Data.Type == TwinsFile.FileType.DemoRM2)
                                    {
                                    ...
                                    if (HasBlendSkin && file.Data.Type == TwinsFile.FileType.DemoRM2)
                                    {
                                    ...
                                    if (ModelList.Count > 0)
                                    {
                                        for (int modelID = 0; modelID < ModelList.Count; modelID++)
                                        {
                                            ...
                                            else if (file.Data.Type == TwinsFile.FileType.RMX)
                                            {
                                                ...
                                                RigidModelController rigidModelCont = null;
                                                SectionController mesh_sec = targetFile.GetItem<SectionController>(11).GetItem<SectionController>(2);
                                                ...
                                                foreach (Twinsanity.RigidModel model in targetFile.Data.GetItem<TwinsSection>(11).GetItem<TwinsSection>(3).Records)
                                                {
                                                    if (model.ID == TargetModel)
                                                    {
                                                        rigidModelCont = rigid_sec.GetItem<RigidModelController>(TargetModel);
                                                ...
                                                if (HasMesh)
                                                {
                                                    ...
                                                    for (int mv = 0; mv < modelCont.Vertices.Count; mv++)
                                                    {
                                                        Vertex[] vbuffer = new Vertex[modelCont.Vertices[mv].Length];
                                                        for (int v = 0; v < modelCont.Vertices[mv].Length; v++)
                                                        {
                                                            vbuffer[v] = modelCont.Vertices[mv][v];
                                                            Vector4 targetPos = new Vector4(modelCont.Vertices[mv][v].Pos.X, modelCont.Vertices[mv][v].Pos.Y, modelCont.Vertices[mv][v].Pos.Z, 1);
        ...
        public void UpdateSelected()
        {
            ...
            else if (file.SelectedItem is InstanceDemo insd)
            {
                SetPosition(new Vector3(-insd.Pos.X, insd.Pos.Y, insd.Pos.Z));
                LoadInstances();
            }
