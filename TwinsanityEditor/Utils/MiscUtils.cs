...
namespace TwinsanityEditor.Utils
{
    internal static class TextUtils
    {
        ...
        {
            System.Drawing.Imaging.BitmapData bitmapBits = data.LockBits(new Rectangle(0, 0, data.Width, data.Height),
        ...
        public static void LoadTexture(uint[] materials, FileController file, VertexBufferData vtx, int index)
        {
            ...
            TwinsSection matSec = file.Data.GetItem<TwinsSection>(secId).GetItem<TwinsSection>(1);
            TwinsSection texSec = file.Data.GetItem<TwinsSection>(secId).GetItem<TwinsSection>(0);
            if (matSec.ContainsItem(material))
            {
                ...
                {
                    Material mat = matSec.GetItem<Material>(material);
                    Texture texture = null;
                    foreach (TwinsShader shader in mat.Shaders)
                    {
                    ...
                    if (texture != null)
                    {
                        Bitmap bmp = texture.GetBmp();
                        int texId = LoadTexture(ref bmp);
                        if (!TextureCache.ContainsKey(material))
                        {
                ...
                {
                    MaterialDemo mat = matSec.GetItem<MaterialDemo>(material);
                    Texture texture = null;
                    foreach (TwinsShader shader in mat.Shaders)
                    {
                        if (shader.TxtMapping == TwinsShader.TextureMapping.ON)
