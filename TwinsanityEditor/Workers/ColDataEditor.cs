...
namespace TwinsanityEditor
{
    public partial class ColDataEditor : Form
    {
        ...
        private void PopulateTree(TreeView trv)
        {
            trv.BeginUpdate();
            trv.Nodes.Clear();
            TreeNode new_node = new TreeNode("Root Node");
            _ = AddTriggerNode(new_node, 0);
            new_node.ExpandAll();
            _ = trv.Nodes.Add(new_node);
            trv.EndUpdate();
        }

        private int AddTriggerNode(TreeNode node, int index)
        {
            if (index >= col.Triggers.Count)
            {
                return -1; //error!
            }

            TreeNode new_node = new TreeNode($"Sub-Node ({index})");
            _ = node.Nodes.Add(new_node);
            ...
            for (index = min_node; index <= max_node && index < col.Triggers.Count; ++index)
            {
                ...
                {
                    _ = new_node.Nodes.Add($"Leaf Node ({index}): Group {-col.Triggers[index].Flag1}");
                }
                else
                {
