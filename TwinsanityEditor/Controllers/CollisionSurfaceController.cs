...
namespace TwinsanityEditor
{
    public class CollisionSurfaceController : ItemController
    {
        ...
        protected override string GetName()
        {
            ...
            return MainFile.Data.Type == TwinsFile.FileType.MonkeyBallRM && Enum.IsDefined(typeof(DefaultEnums.SurfaceTypes_MonkeyBall), surf)
                ? (DefaultEnums.SurfaceTypes_MonkeyBall)surf + " [ID " + Data.ID.ToString() + "]"
                : Enum.IsDefined(typeof(DefaultEnums.SurfaceTypes), surf)
                    ? (DefaultEnums.SurfaceTypes)surf + " [ID " + Data.ID.ToString() + "]"
                    : string.Format("Surface [ID {0:X8}]", Data.ID);
