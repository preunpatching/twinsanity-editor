    ...
    public class FileController : SectionController
    {
        ...
        private void OpenEditor(ref Form editor_var_ptr, Editors editor, Controller cont)
        {
            if (editor_var_ptr == null || editor_var_ptr.IsDisposed)
            {
                switch (editor)
                {
                    ...
                    case Editors.InstanceDemo: editor_var_ptr = new InstanceDemoEditor((SectionController)cont) { Tag = TopForm }; break;
                    case Editors.ObjectDemo: editor_var_ptr = new ObjectDemoEditor((SectionController)cont) { Tag = TopForm }; break;
        ...
        public void CloseEditor(Editors editor, int arg = -1)
        {
            ...
            switch (editor)
            {
                ...
                case Editors.InstanceDemo: editorForm = editInstances[arg]; break; //since arg is -1 by default, an exception will be thrown unless it is specified
